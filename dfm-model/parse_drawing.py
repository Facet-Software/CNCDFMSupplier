...
def parse_drawing(filepath):
    ...
    if not text:
        return {
            ...
            "gdt_frames":           [],
            "radii":                [],
            "surface_finish":       {"detected": False, "values": [], "notes": []},
            "material":             None,
            "datums":               [],
            "process_notes":        [],
            "has_gdt":              False,
...
def _extract_surface_finish(text):
    """
    Detect surface finish callouts including bare Ra values.

    Bare numbers on a drawing near the surface finish symbol (⌓ or ∇)
    are Ra values in µm (metric) or µin (imperial). Common values:
      0.8, 1.6, 3.2, 6.3, 12.5, 25 µm  (metric)
      16, 32, 63, 125, 250, 500 µin     (imperial)
    """
    values = []
    notes = []

    # Explicit Ra: "Ra 1.6" or "1.6 Ra"
    ...
    # Bare standard Ra values on their own line or near surface finish context.
    # Common metric Ra values (µm): 0.1, 0.2, 0.4, 0.8, 1.6, 3.2, 6.3, 12.5, 25
    # These appear as standalone numbers on drawings — a bare "0.8" near no
    # other context is almost certainly a surface finish callout.
    ...
    for pat in [r'ALL\s+SURFACES?\s+\d+\s*(?:RMS|Ra|[µu]in)',
                r'SURFACE\s+FINISH\s*[:=]\s*\d+',
                r'UNLESS\s+OTHERWISE\s+SPECIFIED.*(?:FINISH|SURFACE)']:
        for m in re.finditer(pat, text, re.IGNORECASE):
            notes.append(m.group(0).strip())

    return {"detected": bool(values or notes),
            "values": sorted(set(values)), "notes": notes}
